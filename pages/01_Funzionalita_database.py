...
if uploaded_file_ctrl is not None and st.session_state.get('ctrl_df_sifer_loaded') is None:
    with results_display_area, st.spinner("Elaborazione file CSV SIFER..."):
        try:
            ...
            log_activity("Utente", "SIFER_FILE_VERSION", f"File: {uploaded_file_ctrl.name}, Versione: {sifer_version_row}")
            ...
            log_activity("Utente", "FILE_UPLOADED", f"File: {uploaded_file_ctrl.name}, Righe Dati: {len(df_sifer_raw)}")

            if df_sifer_raw.empty:
                st.error("🚨 CSV SIFER non contiene dati.")
                st.stop()

            df_internal = pd.DataFrame()

            rif_pa_csv = df_sifer_raw['Rif. PA'].iloc[0]
            if not rif_pa_csv or pd.isna(rif_pa_csv):
                st.error("🚨 'Rif. PA' mancante nel CSV SIFER.")
                st.stop()
            is_valid_rif, rif_msg = validate_rif_pa_format(rif_pa_csv)
            if not is_valid_rif:
                st.error(f"🚨 Formato Rif. PA non valido ('{rif_pa_csv}'): {rif_msg}")
                st.stop()
            ...
            if check_rif_pa_exists(current_rif_pa):
                st.error(f"🚫 ATTENZIONE: Registrazione per Rif. PA '{current_rif_pa}' già presente.")
                st.session_state.ctrl_has_blocking_errors = True
                st.stop()
            else:
            ...
            st.error("Il CSV SIFER è vuoto o malformato.")
            st.session_state.ctrl_has_blocking_errors = True
        except pd.errors.ParserError as pe:
            st.error(f"Errore parsing CSV SIFER: {pe}.")
            log_activity("Utente", "CSV_PARSE_ERROR", str(pe))
            st.session_state.ctrl_has_blocking_errors = True
        except ValueError as ve:
            st.error(f"Errore conversione dati da CSV SIFER: {ve}.")
            log_activity("Utente", "DATA_CONVERSION_ERROR", str(ve))
            st.session_state.ctrl_has_blocking_errors = True
        except Exception as e_proc:
            st.error(f"Errore imprevisto: {e_proc}")
            log_activity("Utente", "FILE_PROCESSING_ERROR", str(e_proc))
            st.exception(e_proc)
            st.session_state.ctrl_has_blocking_errors = True
        st.rerun()
...
if st.session_state.get('ctrl_validation_results_df') is not None:
    with results_display_area:
        ...
        if 'rif_pa' in current_rif_pa_info:
            st.subheader(f"📄 Risultati Verifica per Rif. PA: `{current_rif_pa_info['rif_pa']}` (da file SIFER)")
        else:
            st.subheader("🔍 Risultati Verifica Dati Caricati da File SIFER")
    ...
    if st.button("💾 Salva Dati Verificati nel DB", key="save_ctrl_data_btn", type="primary"):
        with results_display_area, st.spinner("Salvataggio nel DB..."):
            ...
            if check_rif_pa_exists(rif_pa_persist):
                 ...
                 log_activity("Utente", "SAVE_BLOCKED_DUPLICATE_RIFPA", f"Rif. PA: {rif_pa_persist}")
            else:
                success_db, msg_db = add_multiple_spese(df_to_persist, "Utente")
                if success_db:
                    st.success(f"✅ {msg_db}")
                    log_activity("Utente", "DATA_SAVED", f"Rif.PA: {rif_pa_persist}, Righe: {len(df_to_persist)}")
                    st.session_state.ctrl_df_sifer_loaded = None
                    st.session_state.ctrl_df_internal_for_validation = None
                    st.session_state.ctrl_df_ready_for_db = None
                    st.session_state.ctrl_validation_results_df = None
                    ...
                    log_activity("Utente", "DATA_SAVE_FAILED", f"Rif.PA: {rif_pa_persist}, Dettaglio: {msg_db}")
